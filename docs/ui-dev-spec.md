...
The history list itself stays as the primary page layout. The summary report is no longer always visible as a fixed right column.

The summary report opens as a fully hidden right-side drawer:

- hidden by default
- opened from each session-app card's `report` action
- closed by clicking the backdrop or the `×` button
- sized to roughly two-fifths of the viewport on desktop

The drawer content uses the shared Markdown Vue renderer and is generated by invoking the project-local Python `analyze.py` from Electron's main process, so the UI reuses the original benchmark report logic instead of maintaining a second renderer-only copy.

If the machine does not provide `python3`, opening a history report must fail loudly in-product:

- show a clear toast
- render the same failure message inside the drawer markdown area
- tell the operator to install Python 3 or fall back to CSV / ZIP export

When samples come from JSONL, each persisted test run must also keep the JSONL source trace (`jsonlPath`, source id, and any lightweight source metadata copied from the sample entry) so later analysis can tell where the alignment text came from even if the current sample library has changed.
...
Opening a session-app card report shows that batch-level summary. Selecting a sample row while the drawer is open switches the drawer to a single-sample deep-dive report with raw text, expected text, latency, retry count, and error-rate notes when `expectedText` is available.

The batch-level report should mirror the structure of the original `analyze.py` output for the single selected app as closely as possible, including:
...
- hover-revealed `report` action
- hover-revealed icon-only ZIP export action
- hover-revealed `展开 / 收起` action

History cards are collapsed by default. Entering `测试历史` must not auto-expand the first card.
...
The report drawer header stays minimal:

- no repeated app-title pill
- no repeated app-version pill
- icon-only report export action with tooltip
- close affordance as `×`
